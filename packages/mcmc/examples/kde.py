...
##############################################################################
# If we want the final interval to be [1.2, 2.5], then l = 1.3
true_params = [1.2, 1.3]  
dist_data = granulometre.distribution.Uniform(true_params)

# Use the existing Simulator to draw N=100 samples of ellipsoids -> (Dmax,Dmin)
...
##############################################################################
# 2) Define a prior: 0 <= u <= 10, 0 <= l <= 10
##############################################################################
...
dist_inference = granulometre.distribution.Uniform(theta_inference)
sim_inference  = granulometre.Simulator(dist_inference)

##############################################################################
# 4) Define a likelihood (KDE) that expects param_names=["u","l"]
##############################################################################
param_names = ["u","l"]
...
print(f"Posterior mean u: {u_mean:.3f}, l: {l_mean:.3f}")
